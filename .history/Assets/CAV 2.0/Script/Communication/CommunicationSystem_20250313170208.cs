...
public class CommunicationSystem : MonoBehaviour
{
    ...
    private void InitializeRoadSegments()
    {
        ...
        foreach (Segment segment in trafficSystem.segments)
        {
            float segmentLength = Vector3.Distance(
                ...
                segment.waypoints[1].transform.position
            );
            roadSegments.Add(new RoadSegment(segment.id, segment.waypoints[0].transform.position, segment.waypoints[^1].transform.position, segmentLength));
        }
    ...
    public class RoadSegment
    {
        ...
        public void ReceiveInformation(string message, int senderId)
        {
            ...
            if (message == "enter") AddVehicle(senderId);
            else if (message == "exit") RemoveVehicle(senderId);
        }
    }
