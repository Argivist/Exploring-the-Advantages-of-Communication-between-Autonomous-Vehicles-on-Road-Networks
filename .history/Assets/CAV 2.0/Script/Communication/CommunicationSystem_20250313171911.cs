    ...
    public class RoadSegment
    {
        ...
        public void UpdateData(int vehicleId, float speed, int time)
        {
            ...
            if (data != null)
            {
                ...
                data.time = time;
                UpdateAverages();
                dynamicCost = (length / totalLength) * (avgTime / (length / maxSpeed)) / avgSpeed;
            }
        }
        ...
        public void ReceiveInformation(string message, int senderId, float speed, int time)
        {
            UpdateData(senderId, speed, time);
                    }
    }
