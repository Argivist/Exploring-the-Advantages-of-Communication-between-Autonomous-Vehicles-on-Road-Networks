...
public class VehicleSpawnerObject : MonoBehaviour
{
    ...
    void Update()
    {
        if(canSpawn){
            // Spawn vehicle
            GameObject vClone=Instantiate(vehicleObject, this.transform.position, Quaternion.identity);
            Debug.Log("Pos and Rot: "+this.transform.position+" "+this.transform.rotation);
