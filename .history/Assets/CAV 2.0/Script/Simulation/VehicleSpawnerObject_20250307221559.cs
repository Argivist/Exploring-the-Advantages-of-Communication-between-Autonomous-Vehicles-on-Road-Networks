...
public class VehicleSpawnerObject : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //list all waypoints in start segment
        for(int i=0; i<startSegment.waypoints.Count; i++){
            Debug.Log("Waypoint: "+startSegment.waypoints[i].transform.position);
        }
        direction();
    ...
    void direction(){
        //Find nearest waypoint to start within the segment
                    ...
                    for(int j=0; j<startSegment.waypoints.Count; j++){
                        Debug.Log("Waypoint: "+startSegment.waypoints[j].transform.position);
