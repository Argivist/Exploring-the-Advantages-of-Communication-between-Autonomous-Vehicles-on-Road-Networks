...
File.WriteAllText(Path.Combine(Application.persistentDataPath, safeFileName), json);
Debug.Log($"Network data saved to {Path.Combine(Application.persistentDataPath, safeFileName)}");
        ready = true;
        yield return null;
    }
