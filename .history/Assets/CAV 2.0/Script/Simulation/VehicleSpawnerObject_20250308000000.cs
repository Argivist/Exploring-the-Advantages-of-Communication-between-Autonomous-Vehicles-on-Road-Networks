...
public class VehicleSpawnerObject : MonoBehaviour
{
        ...
        //Find nearest waypoint to start within the segment
                    ...
                    for(int j=0; j<startSegment.waypoints.Count; j++){
                        ...
                        if(d < minDist && lSpace.z > 0){
                            ...
                            startRotation.eulerAngles=Vector3.up*angle;
                        }
                    }
                    canSpawn=true;
    ...
    void Update()
    {
        if(canSpawn){
            // Spawn vehicle
            GameObject vClone=Instantiate(vehicleObject, this.transform.position,Quaternion.Euler(startRotation));
            Debug.Log("Pos and Rot: "+this.transform.position+" "+startRotation.eulerAngles);
