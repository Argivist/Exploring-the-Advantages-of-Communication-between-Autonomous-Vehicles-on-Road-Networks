...
public class VehicleSpawnerObject : MonoBehaviour
{
    ...
    void Update()
    {
        if(canSpawn){
            // Spawn vehicle
            GameObject vClone=Instantiate(vehicleObject, this.transform.position,Quaternion.Euler(0,angle,0));
            Debug.Log("Pos and Rot: "+this.transform.position+" "+Quaternion.Euler(0,angle,0));
