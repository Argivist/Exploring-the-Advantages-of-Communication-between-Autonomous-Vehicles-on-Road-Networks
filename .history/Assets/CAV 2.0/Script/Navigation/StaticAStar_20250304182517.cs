...
// }
    ...
    public void AStarPathfinder(Dictionary<int, Segment> segments)
    {
    ...
    //         foreach (int nextID in segments[currentID].nextSegments.Select(segment => segment.id))
    //         {
...
public List<int> FindPath(int startID, int goalID)
{
    ...
    while (openSet.Count > 0)
    {
        ...
        foreach (Segment nextSegment in segments[currentID].nextSegments) // Iterate through Segment objects
        {
            int nextID = nextSegment.id;  // Extract ID
            float tentativeGScore = gScore[currentID] + nextSegment.cost; // Use nextSegment.cost
