...
namespace TrafficSimulation
{
    ...
    */
    ...
    public class Vehicle_AI : MonoBehaviour
    {
        ...
        {
            Debug.Log("Vehicle id: " + GetComponent<Description>().id + " Destroyed");
            int time = StopWatch.getTime();
            // Debug.Log("Time: "+time+"forvehicle id: "+GetComponent<Description>().id);
            StopWatch.stopTimer();
            // Debug.Log("Time: stopped");
            SimulationMaster_ sm = GameObject.FindObjectOfType<SimulationMaster_>();
            // Debug.Log("SimulationMaster_ foundin vehicle id: "+GetComponent<Description>().id);
            sm.VehicleDestroyed(GetComponent<Description>().id, time);
            // Debug.Log("VehicleDestroyed called in vehicle id: "+GetComponent<Description>().id);
            Destroy(this.gameObject);
        ...
        void WaypointChecker()
        {
            ...
            if (wpDist.magnitude < waypointThresh)
            {
                ...
                // Ensure valid waypoint index,if the current target is the last waypoint in the segment, get the next segment
                if (currentTarget.waypoint >= trafficSystem.segments[currentTarget.segment].waypoints.Count)
                {
                    pastTargetSegment = currentTarget.segment;
                    currentTarget.segment = NavigationComponent.GetNextSegmentId();
                    ...
                    // Ensure valid segment before resetting waypoint index
                    // if (currentTarget.segment < 0 || currentTarget.segment >= trafficSystem.segments.Count)
                    // {
                    //     Debug.LogError($"Invalid new segment: {currentTarget.segment}. Segments count: {trafficSystem.segments.Count}");
                    //     return;
                    // }
                ...
                if (futureTarget.waypoint >= trafficSystem.segments[currentTarget.segment].waypoints.Count)
                {
                    ...
                    if (NavigationComponent.path.Count > 1)
                    {
                        futureTarget.segment = NavigationComponent.path[1];
                    }
                    else
                    {
                        Debug.LogWarning("NavigationComponent.path has fewer than 2 elements!");
                    }
                }
            }
        }
        ...
        void MoveVehicle()
        {
            ...
            if (currentTarget.segment < 0 || currentTarget.segment >= trafficSystem.segments.Count)
            {
                Debug.LogError($"MoveVehicle Error: Invalid segment index {currentTarget.segment}. Segment count: {trafficSystem.segments.Count}");
                return;
            }

            if (currentTarget.waypoint < 0 || currentTarget.waypoint >= trafficSystem.segments[currentTarget.segment].waypoints.Count)
            {
                Debug.LogError($"MoveVehicle Error: Invalid waypoint index {currentTarget.waypoint}. Waypoint count: {trafficSystem.segments[currentTarget.segment].waypoints.Count}");
                return;
            }

            if (futureTarget.segment < 0 || futureTarget.segment >= trafficSystem.segments.Count)
            {
                Debug.LogError($"MoveVehicle Error: Invalid future segment index {futureTarget.segment}. Segment count: {trafficSystem.segments.Count}");
                return;
            }

            if (futureTarget.waypoint < 0 || futureTarget.waypoint >= trafficSystem.segments[futureTarget.segment].waypoints.Count)
            {
                Debug.LogError($"MoveVehicle Error: Invalid future waypoint index {futureTarget.waypoint}. Waypoint count: {trafficSystem.segments[futureTarget.segment].waypoints.Count}");
                return;
            }
        ...
        void SetWaypointVehicleIsOn()
        {
            ...
            {
                futureTarget.waypoint = 0;
                futureTarget.segment = NavigationComponent.path[1];// GetNextSegmentId();// TODO: USE GET SEGMENT HERE SO THE NEXT SEGMENT IS SET AND THEN CORRECT THE WAYPOINT CHECKER
