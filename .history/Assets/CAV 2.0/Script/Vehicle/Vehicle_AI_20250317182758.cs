    ...
    */
    ...
    void SetWaypointVehicleIsOn()
        ...
        void MoveVehicle()
        {
            ...
            if (currentTarget.segment < 0 || currentTarget.segment >= trafficSystem.segments.Count)
            {
                Debug.LogWarning($"MoveVehicle Error: Invalid segment index {currentTarget.segment}. Segment count: {trafficSystem.segments.Count}");
                return;
            }

            if (currentTarget.waypoint < 0 || currentTarget.waypoint >= trafficSystem.segments[currentTarget.segment].waypoints.Count)
            {
                Debug.LogWarning($"MoveVehicle Error: Invalid waypoint index {currentTarget.waypoint}. Waypoint count: {trafficSystem.segments[currentTarget.segment].waypoints.Count}");
                return;
            }

            if (futureTarget.segment < 0 || futureTarget.segment >= trafficSystem.segments.Count)
            {
                Debug.LogWarning($"MoveVehicle Error: Invalid future segment index {futureTarget.segment}. Segment count: {trafficSystem.segments.Count}");
                return;
            }

            if (futureTarget.waypoint < 0 || futureTarget.waypoint >= trafficSystem.segments[futureTarget.segment].waypoints.Count)
            {
                Debug.LogWarning($"MoveVehicle Error: Invalid future waypoint index {futureTarget.waypoint}. Waypoint count: {trafficSystem.segments[futureTarget.segment].waypoints.Count}");
                return;
            }
