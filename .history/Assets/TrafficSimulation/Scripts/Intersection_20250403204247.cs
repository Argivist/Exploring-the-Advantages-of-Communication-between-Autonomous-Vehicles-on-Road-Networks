...
public class VehicleDebug
{
    ...
    public VehicleDebug(GameObject _vehicle, Status_ _status, bool _isAlreadyInIntersection, bool _isInQueue, bool _priority, int _segmentVehicleIsIn, int _nextSegment)
    {
        vehicle = _vehicle;
        status = _status;
...
namespace TrafficSimulation
{
    ...
    public class Intersection : MonoBehaviour
    {
        ...
        public List<GameObject> ExitSegments = new List<GameObject>();
        public List<GameObject> EntrySegments = new List<GameObject>();
        ...
        public List<GameObject> GetVehiclesInIntersection() => vehiclesInIntersection;
        public List<GameObject> GetVehiclesQueue() => vehiclesQueue;

        void Update()
        {
            if (intersectionType == IntersectionType.STOP)
            {
                // Remove null vehicles from the intersection
                for (int i = vehiclesInIntersection.Count - 1; i >= 0; i--)
                {
                    GameObject vehicle = vehiclesInIntersection[i];
                    if (vehicle == null)
                    {
                        vehiclesInIntersection.RemoveAt(i);
                        continue;
                    }
                ...
                for (int i = vehiclesQueue.Count - 1; i >= 0; i--)
                {
                    GameObject vehicle = vehiclesQueue[i];
                    if (vehicle == null)
                    {
                        vehiclesQueue.RemoveAt(i);
                        continue;
                    }
                ...
                // If queue is not empty and intersection is free, move the first vehicle
        ...
        {
            if (_other.CompareTag("Waypoint") && _other.gameObject.name.Contains("0"))
            {
                ExitSegments.Add(_other.gameObject.transform.parent.gameObject);
            }

                if (IsAlreadyInIntersection(_other.gameObject) || Time.timeSinceLevelLoad < 0.5f) return;
            ...
            if (_other.CompareTag("AutonomousVehicle"))
            {
                if (intersectionType == IntersectionType.STOP)
                    TriggerStop(_other.gameObject);
                else if (intersectionType == IntersectionType.TRAFFIC_LIGHT)
        ...
        void ExitStop(GameObject _vehicle)
        {
            ...
            vehiclesList.RemoveAll(x => x.vehicle == _vehicle);
        ...
        {
            List<Segment> currentLightsGroup = (currentRedLightsGroup == 1) ? lightsNbr1 : lightsNbr2;
            return currentLightsGroup.Exists(segment => segment.id == _vehicleSegment);
        }
    }
}
...
//         void OnTriggerEnter(Collider _other)
//         {
//             if (_other.tag == "Waypoint" && _other.gameObject.name.Contains("0"))
//             {
//                 ExitSegments.Add(_other.gameObject.transform.parent.gameObject);
//             }
...
//         //     if(_other.tag == "Waypoint"){
//         //         // Debug.Log("Waypoint entered");
//         //         // Debug.Log(_other.gameObject.name);
//         //         // Debug.Log(_other.gameObject.GetComponent<Segment>().id);
//         //         // EntrySegments.Add(_other.gameObject.GetComponent<Segment>());

//         //         Debug.LogWarning("Intersect: "+id+" Entry: "+_other.gameObject.GetComponent<Segment>().id+"Waypoint: "+_other.gameObject.name);
//         //     }
//         // }
